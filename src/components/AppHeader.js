...
import { ButtonGroup, AppBar, Box, Toolbar, IconButton, Typography } from '@mui/material';
...
const AppHeader = () => {
  ...
  const pages = [
    { label: Strings.users, path: '/users' },
    { label: Strings.pl_table, path: '/premier-league-table' }
  ];
  const settings = [
    {
        ...
        <Toolbar disableGutters>
          ...
          <Box sx={{ flexGrow: 0 }}>
            <ButtonGroup sx={{ mr: 2 }} variant='contained' size='small'>
              <Button color={language === 'EN' ? 'secondary' : 'disabled'} onClick={() => setLanguage('EN')}>
                {Strings.english}
              </Button>
              <Button color={language === 'VI' ? 'secondary' : 'disabled'} onClick={() => setLanguage('VI')}>
                {Strings.vietnamese}
              </Button>
            </ButtonGroup>
            ...
            <Menu sx={{ mt: 1 }} anchorEl={anchorElUser} keepMounted open={Boolean(anchorElUser)} onClose={handleCloseUserMenu}>
